...
| `label` | `string` | Any string | NA | This prop will provide a label for the input field. If you want to align the `label` to the right, then you can wrap a `<CurrencyInput>` component in a `<div>` and specify `text-align: right` on that `<div>`. |
