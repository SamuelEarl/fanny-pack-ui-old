...
let { svgWidth, svgHeight, hoveredValueXPos, data, xValueId, xScaleFunction, yScaleFunction } = getContext(AREA_CHART_KEY);
$: xScale = xScaleFunction($svgWidth);
$: yScale = yScaleFunction($svgHeight);
$: areaGenerator = area().x((d) => xScale(d[xValueId])).y0(() => yScale.range()[0]).y1((d) => yScale(d[yValueId])).curve(curveCatmullRom.alpha(0.5));
$: lineGenerator = line().x((d) => xScale(d[xValueId])).y((d) => yScale(d[yValueId])).curve(curveCatmullRom.alpha(0.5));
function createId() {
