...
const { accordionTitlePadding, fontSize, spaceBetweenAccordions } = getContext(ACCORDION_KEY);
...
<style>
    ...
    .fp-accordion-wrapper:last-child {
      margin-bottom: 0 !important;
    }.fp-accordion-wrapper .fp-accordion-title {
      border: var(--border-default);
      border-radius: var(--border-radius);
      background-color: var(--bg-color-element-default);
      color: var(--text-color-default);
      cursor: pointer;
    }.fp-accordion-wrapper .fp-accordion-title:after {
        content: "+"; /* Unicode character for "plus" sign (+) */
        font-weight: bold;
        color: inherit;
        float: right;
        margin-left: 5px;
      }.fp-accordion-wrapper .fp-accordion-title.active:after {
        content: "−"; /* Unicode character for "minus" sign (-) */
      }.fp-accordion-wrapper .fp-accordion-title:hover {
